...
public class FailureWriterReadScenario {

	private static final int NUM_NODES = 10;
	
	public static final int EXECUTION_PROCESS_COUNT = 8;
		...
		public StartNodeEvent generate() {
			return new StartNodeEvent() {
				...
				public Class getComponentDefinition() {
					return FailureWriterReadObserver.class;
				}
	...
	static Operation1 startWriteClient = new Operation1<StartNodeEvent, Integer>() {
		...
		public StartNodeEvent generate(final Integer writerId) {
			return new StartNodeEvent() {
				...
				{
					try {
						selfAdr = new TAddress(InetAddress.getByName("10.19.0.1" + writerId), 10000);
						dest = new TAddress(InetAddress.getByName("10.19.0." + writerId), 10000);
					} catch (UnknownHostException ex) {
						throw new RuntimeException(ex);
					}
				...
				@Override
				public Init getComponentInit() {
					return new WriteClient.Init(selfAdr, dest, DATA_KEY,  writerId == 1 ? DATA_VALUE1 : DATA_VALUE2, writerId == 1 ? false : true);
				}
		...
		SimulationScenario scenario = new SimulationScenario() {
			{
				...
				StochasticProcess startReadClients22 = new StochasticProcess() {
					{
						eventInterArrivalTime(constant(1000));
						raise(1, startReadClient, new BasicIntSequentialDistribution(2));
					}
				...
				StochasticProcess startReadClients32 = new StochasticProcess() {
					{
						eventInterArrivalTime(constant(1000));
						raise(1, startReadClient, new BasicIntSequentialDistribution(3));
					}
				...
				startWriteClients2.startAfterTerminationOf(1000, startWriteClients1);
				startReadClients2.startAfterTerminationOf(1000, startWriteClients2);
				startReadClients3.startAfterTerminationOf(2000, startReadClients2);

				terminateAfterTerminationOf(100000, startReadClients3);
			}
		};
		return scenario;
	}
